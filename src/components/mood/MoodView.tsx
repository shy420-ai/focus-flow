...
function EntryCard({ entry, onTap, onDelete }: { entry: MoodEntry; onTap: () => void; onDelete: () => void }) {
  ...
  const dropDelta = (entry.distressBefore != null && entry.distressAfter != null)
    ...
    : null
  const stateLine = [
    entry.focus != null ? `집중 ${entry.focus}` : null,
    entry.mood != null ? `기분 ${entry.mood}` : null,
    entry.energy != null ? `에너지 ${entry.energy}` : null,
  ].filter(Boolean).join(' · ')
  ...
  return (
    ...
    >
      ...
      {stateLine && (
        <div style={{ fontSize: 10, color: '#888', marginBottom: 2 }}>{stateLine}</div>
      )}
      {summary && (
        <div style={{ fontSize: 12, color: '#333', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{summary}</div>
      )}
